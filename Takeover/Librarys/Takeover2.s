...
.loop		bra.s	.loop
		...
		move.l	chip_chunk_address(a5),a0
		move.w	#4,d0
		move.w	#320,d1
		move.w	#200,d2
		move.l	#0,d3
...
.error		movem.l	(sp)+,d1-d7/a1-a6
		...
		ERROR_DEFN <"ERROR #00041 : Not enough temporary memory">,24,17
...
.cli
		...
		WRITEINFO <"              Temp chunk address">,<"UNKNOWN ">,temp_chunk_address(a5)
...
.loop
		...
		jsr	_LVOWaitTOF(a6)
		movem.l  (sp)+,d1/a5
		movem.l	d1/a5,-(sp)		;clear screen 1 bitmap
		lea	screen1_bitmap_structure(pc),a0
		move.l	bm_Planes(a0),a1	;address to clear
		move.w	bm_BytesPerRow(a0),d0
		mulu.w	bm_Rows(a0),d0
		move.b	bm_Depth(a0),d1
		ext.w	d1
		mulu.w	d1,d0			;bytes to clear
		move.l	#$fffffffd,d1		;wait until done
		move.l	graphics_base(a5),a6
		jsr	_LVOBltClear(a6)        ;do blit
		movem.l	(sp)+,d1/a5
		move.w	#$f00,$dff180
		move.w	#500,d0
.delay		dbf	d0,.delay
		...
		movem.l	d1/a5,-(sp)		;clear screen 2 bitmap
		lea	screen2_bitmap_structure(pc),a0
		move.l	bm_Planes(a0),a1	;address to clear
		move.w	bm_BytesPerRow(a0),d0
		mulu.w	bm_Rows(a0),d0
...
parameters	dc.b	"Hired Guns",0,0,0,0,0,0
		...
		dc.l	100*1024	;chip memory maximum amount
		dc.l	100*1024	;public memory maximum amount
